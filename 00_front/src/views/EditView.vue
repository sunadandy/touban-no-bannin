...
export default {
    ...
    onSubmit() {
      ...
      switch (this.result.tab) {
        ...
        case "option-2":
          ...
          // メンバーの更新
          var updateMemberInfos = this.$refs.RefCaclcMemberInfo.ReplaceMember(
            this.current_memberInfos,
            selected
          );
          // 変更したメンバーの次回実施予定日設定
          ...
          updateMemberInfos = this.$refs.RefCaclcMemberInfo.ReSchedule(
            updateMemberInfos,
            newInterval,
            newWeek,
            newDay,
          ...
          // 古いメンバー情報を削除
          this.axios
            .delete("/member", { params: { id: this.toubanId } })
            .then((response) => {
              // 新しいメンバー情報を登録
        ...
        case "option-3":
          ...
          var newDay = parseInt(data[2]);
          var newDate = parseInt(data[3]);
          var updateMemberInfos = this.$refs.RefCaclcMemberInfo.ReSchedule(
            this.current_memberInfos.sort(
        ...
        case "option-6":
          ...
          updateToubanInfo.cc =
            updateToubanInfo.cc + ";" + this.result.data.email;
          this.axios
            .put("/touban", updateToubanInfo)
            .then((response) => {
              console.log(response);
  ...
  created() {
    ...
    this.current_memberInfos = this.$store.getters.GetMemberByToubanId(
      this.toubanId
    );
  },
};
